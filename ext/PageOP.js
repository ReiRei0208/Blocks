(function(Scratch) {
  ...
  class StrictEqualityExtension {
	getInfo() {
      ...
      },  
      {
        opcode: 'l',
        blockType: Scratch.BlockType.COMMAND,
        text: '绘制宽度为 [width] [color] 的线:[align]',
        arguments: {
            width: {
  ...
  l(args) {
    if ( args.align == 'left' || args.align == 'right' || args.align == 'center' ) {
        document.write( '<hr width="' + args.width + '" color="' + args.color +'" align="' + args.align + '" noshade>' )
    }else{
        document.write( '<span style="color:darkred">Error：align 值不正确，应为“left”、“right”或者“center”</span>' );
        return 'Error';
    }
}
  h(args) {
